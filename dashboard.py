...
<style>
    ...
    [data-testid="stMetric"] {
        ...
        border: 1px solid #1e293b; border-top: 3px solid #3b82f6;
    ...
    .terminal-feed {
        ...
        border: 1px solid #1e293b; border-left: 3px solid #10b981;
        border-radius: 4px; color: #a3e635;
    ...
    .svc-card { border: 1px solid #1e293b; padding: 10px; border-radius: 4px; background: #0f172a; margin: 4px 0; font-family: monospace; font-size: 0.8rem; }
...
def draw_service_mesh(nodes_state: dict) -> go.Figure:
    ...
    STATUS_UI = {
        ...
        "compromised": {"color": "#ef4444", "size": 50, "tag": "[BREACHED]"},
    ...
    edge_trace = go.Scatter(x=ex, y=ey, line=dict(width=1.5, color="#475569"), mode="lines", hoverinfo="none")
    node_trace = go.Scatter(
        x=[pos[n][0] for n in G.nodes()], y=[pos[n][1] for n in G.nodes()],
        mode="markers+text", text=node_texts, hovertext=hover_texts,
        textposition="bottom center",
        textfont=dict(size=11, color="#e2e8f0", family="monospace"),
...
services_data, svc_ok  = fetch_api("services")

if not state_ok or not hist_ok:
    st.markdown("<div class='offline-banner'>CRITICAL: TELEMETRY LINK SEVERED. ATTEMPTING RECONNECTION...</div>", unsafe_allow_html=True)
else:
    st.markdown(
        f"<div style='color:#10b981; font-size:0.85rem; font-family:monospace; margin-bottom:10px;'>"
        f"[ {datetime.now().strftime('%H:%M:%S')} ] SECURE CONNECTION ESTABLISHED // LIVE FEED</div>",
        unsafe_allow_html=True,
    )
...
c1.metric("GLOBAL UPTIME",  f"{uptime:.1f}%",
          delta=f"{uptime - 100:.0f}%" if uptime < 100 else "NOMINAL", delta_color=uptime_color)
...
with col_left:
    ...
    st.markdown("<h3>LIVE MICROSERVICE HEALTH</h3>", unsafe_allow_html=True)
    if svc_ok and services_data:
        for svc_name, svc_info in services_data.items():
            reachable = svc_info.get("reachable", False)
            status    = svc_info.get("status", "offline")
            color     = "#10b981" if status == "healthy" else "#ef4444" if status in ("compromised", "degraded") else "#f59e0b"
            latency   = svc_info.get("latency_ms", "N/A")
            st.markdown(
                f"<div class='svc-card'>"
                f"<span style='color:{color}; font-weight:bold;'>[{status.upper()}]</span> "
                f"{svc_name.upper()} &nbsp;|&nbsp; latency: {latency}ms"
                f"</div>",
                unsafe_allow_html=True,
            )

    st.markdown("<h3>ACTIVE THREAT INTEL</h3>", unsafe_allow_html=True)
    ...
    if alerts:
        for alert in alerts:
            ...
            st.markdown(
                f"<div class='{css_class}'>[{sev}] {alert['target_node'].upper()} :: {symptom}</div>",
                unsafe_allow_html=True,
            )
    else:
...
with col_mid:
    ...
    if "APPROVED" in output:
        st.markdown(
            f"<div class='judge-box' style='border-color:#10b981; background:rgba(16,185,129,0.05);'>"
            f"<div class='judge-title' style='color:#34d399;'>VERDICT: APPROVED</div>"
            f"<span style='font-family:monospace; font-size:0.85rem;'>{output}</span></div>",
            unsafe_allow_html=True,
        )
    elif "REJECTED" in output:
        st.markdown(
            f"<div class='judge-box' style='border-color:#ef4444; background:rgba(239,68,68,0.05);'>"
            f"<div class='judge-title' style='color:#fca5a5;'>VERDICT: REJECTED</div>"
            f"<span style='font-family:monospace; font-size:0.85rem;'>{output}</span></div>",
            unsafe_allow_html=True,
        )
    elif "PENALTY" in output or "VIOLATION" in output or "DENIED" in output:
        st.markdown(
            f"<div class='judge-box' style='border-color:#ef4444; background:rgba(239,68,68,0.1);'>"
            f"<div class='judge-title' style='color:#fca5a5;'>POLICY VIOLATION</div>"
            f"<span style='font-family:monospace; font-size:0.85rem;'>{output}</span></div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            "<div class='judge-box'><div class='judge-title'>AWAITING TICKET SUBMISSION</div>"
            "<span style='font-family:monospace; color:#64748b; font-size:0.85rem;'>"
            "Agent has not yet submitted forensic evidence to the Change Board.</span></div>",
            unsafe_allow_html=True,
        )
...
with col_right:
    ...
    if history:
        for action in reversed(history):
            ...
            audit_html += (
                f"<span style='color:#3b82f6;'>[STEP {action['step']:02d}]</span> "
                f"<strong style='color:#e2e8f0;'>{action['action'].upper()}</strong><br>"
                f"<span style='color:#64748b;'>TARGET:</span> {action['target'][:60]}<br>"
                f"<span style='color:#64748b;'>REWARD:</span> "
                f"<span style='color:{color}; font-weight:bold;'>{r_val:+.1f}</span> :: "
                f"{action['message'][:80]}<br><br>"
            )
    else:
