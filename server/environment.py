...
class ZeroTrustEnv(_OpenEnvBase):
    ...
    def step(self, action: Action) -> StepResult:
        ...
        if tool == "query_siem_logs":
            ...
            self.state.command_output = (
                f"Error: tool '{tool}' not recognised. "
                f"Use query_siem_logs, file_ticket, check_approval, or isolate_node."
            )
    ...
    def state_dict(self) -> dict:
        ...
        return {
            ...
            "remaining_compromised":    self._remaining_compromised,
            "difficulty":               self.difficulty,
            "judge_persona":            self.judge_persona,
            ...
            "ticket_approved":          self.ticket_approved,
            "siem_queried_nodes":       list(self.siem_queried_nodes),
            "curriculum":               self.curriculum.get_summary(),
            "cve_context":              self.cve_context,
    ...
    def _pull_real_logs(self, node: str) -> str:
        ...
        if not port:
            return ""
        try:
            resp = _requests.get(f"http://localhost:{port}/logs", timeout=2)
    ...
    def _fake_timestamp(self) -> str:
        base = datetime.datetime.now() - datetime.timedelta(minutes=random.randint(2, 45))
